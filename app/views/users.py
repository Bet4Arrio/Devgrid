...
async def post_cities():
    ...
    req = User_requests(req_id, num_cities, time)
    
    try:
        result = save_request(req)
        tasks = []
        for city in cities:
            task = asyncio.create_task(new_city(req_id, city))
            tasks.append(task)
        cit = await asyncio.gather(*tasks)
        return jsonify({'message': 'successfully registered', 'session': result, 'data':cit}), 201
    except:
...
def get_request(id):
    ...
    if req:
        ...
        user = user_request_schema.dump(request_by_id(id))
        pct = (len(result)/user["num_cities"])*100
        return jsonify({'message': f'{pct}%complete', 'data': result})
